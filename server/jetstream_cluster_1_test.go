...
func TestJetStreamClusterStreamHealthCheckOnlyReportsSkew(t *testing.T) {
	...
	node.Stop()
	node.WaitForStop()
	node, err = sjs.createRaftGroup(globalAccountName, group, nil, false, FileStorage, pprofLabels{})
	require_NoError(t, err)
...
func TestJetStreamClusterConsumerHealthCheckMustNotRecreate(t *testing.T) {
	...
	_, err = sjs.createRaftGroup(globalAccountName, ca.Group, nil, false, MemoryStorage, pprofLabels{})
	require_NoError(t, err)
	sjs.mu.Lock()
